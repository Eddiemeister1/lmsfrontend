...
export class CourseService {
  ...
  getGroupByCourseId(id) {
    return this.http.get(`${environment.apiURL}/courses/group/${id}`);
  }


  addCourse(name, description, seats, tgroup, start_date, end_date) {
    const course = {
      name: name,
  ...
  updateCourse(id, name, description, seats, tgroup, start_date, end_date) {
    const course = {
      name: name,
      description: description,
      seats: seats,
  ...
  updateCourseGroup(id, tgroup){
    const seatObject = {
      tgroup: tgroup
    };
    return this.http.put(`${environment.apiURL}/courses/groupUpadate/${id}`, seatObject);
  }
