...
export class EditModuleComponent implements OnInit {
  ...
  constructor(private moduleService: ModuleService, private router: Router, private route: ActivatedRoute, private fb: FormBuilder) {
   ...
   CreateForm() {
    this.updateForm = this.fb.group({
      ...
      extraCreditPoints: ['', Validators.required],
      lockedUntil: ['']
    })
  }
  ...
  ngOnInit() {
    this.route.params.subscribe(params => {
      ...
      this.moduleService.getModuleById(this.id).subscribe((res: Module) => {
        ...
        this.updateForm.get('extraCreditPoints').patchValue(this.module.extraCreditPoints);
  ...
  updateModule(number, title, extraCreditPoints, lockedUntil) {
    this.submitted = true;
    ...
    if(!this.updateForm.valid){
      ...
      this.moduleService.updateModule(this.id, number, title, extraCreditPoints, lockedUntil).subscribe(res => {
